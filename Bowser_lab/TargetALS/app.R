    ...
    mainPanel(
      tabsetPanel(
        tabPanel("Real data - Enrolled subjects",
                 ...
                 h6(""),
                 radioButtons("time","Choose month/year",c("Month","Year"), selected = "Month"),
        ...
        tabPanel("Predicted graph - Visits",
                 ...
                 h6("2. Controls visits twice in a year - 1st month and 12th month"),
                 br(),
...
server <- function(input,output){
  ...
  #data<- real_data %>% dplyr::select(type="Control",Dates)
  ...
  final_data = reactive({
    ...
    total_visit_ALS<-function(x)
    {
      ...
      if(!!input$group=="ALS"){
        ...
        d<-ifelse(c>=12|c==0,as.integer(c/12)+1,1)
        e<-ifelse(d>=2,2,d)
      }
      ...
      f<- df%>%
        ...
        mutate(plasma=22*visits*totalnos,serum=16*visits*totalnos,urine=11*visits*totalnos,
               ...
               dna=2*totalnos, rna= totalnos, total_dna_rna=3*totalnos)
  ...
  output$NOS_barplot <- renderUI({
    if(input$time=="Month"){
      ...
      gg<- data() %>%
        ...
        ggplot(aes(x=year,y=total, color=total, fill=total))+
        ...
        theme(axis.text.x = element_text(angle=90,vjust=0.5),
              ...
              legend.title = element_text(size=10),
              legend.text = element_text(size=10))+
        labs(title=paste("No. of subjects enrolled"), x="",y="No.of subjects(n)")
      renderPlot(gg)
    }
  })
  ...
  output$NOS_barplot2 <- renderUI({
    ...
    else
    {
      gg<- rd %>%
        ...
        group_by(year,Group) %>%
        summarise(total=sum(nos))%>%
        ggplot(aes(x=year, y=total, color=Group, fill=Group))+
        ...
        theme(axis.text.x = element_text(angle=90,vjust=0.5),
              ...
              legend.title = element_text(size=10),
              legend.text = element_text(size=10))+
        labs(title=paste("No. of subjects enrolled"), x="Year",y="No.of subjects(n)")
      renderPlot(gg)
    }
  })
